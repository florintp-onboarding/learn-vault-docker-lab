...
resource "null_resource" "active_node_init" {
  provisioner "local-exec" {
    command = "while ! curl --insecure --fail --silent https://127.0.0.1:8200/v1/sys/seal-status --output /dev/null ; do printf '.' ; sleep 4 ; done ; vault operator init -t 1 -n 1 > ${path.cwd}/.vault_docker_lab_1_init"
    environment = {
